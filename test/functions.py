...
def wce_dice_huber_loss(pred, gt):             #这个pred是如何生成的，gt又是什么？
    # print(gt.shape)
    # print(pred.shape)
...
def wce_dice_huber_loss_2(pred, band_gt,dou_gt):
    loss1 = cross_entropy_loss(pred, band_gt)
    loss3 = cross_entropy_loss(pred, dou_gt)
    loss2 = DiceLoss()(pred, band_gt)
    loss4 = DiceLoss()(pred, dou_gt)
    # loss3 = smooth_l1_loss(pred, gt)
    return 0.6 * (loss1+loss3) + 0.4 * (loss2+loss4)
...
def cross_entropy_loss(prediction, label):
    label = label.long()
    mask = (label != 0).float()                                            #这个mask为什么会变？？？
